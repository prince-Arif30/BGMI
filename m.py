...
bot = telebot.TeleBot('7514794643:AAGEGpdk_GPJtVHCDd3NC_guUQ_-toXtZd4')
...
✅ Usage: /remove <userid>'''
        ...
        response = "You have not purchased yet purchase now from:- @Dadaxyt❣️"
...
# Add /mylogs command to display logs recorded for bgmi command website commands
    ...
    for handler in bot.message_handlers:
        if hasattr(handler, 'commands'):
            if message.text.startswith('/attack'):
                attack_text += f"{handler.commands[0]}: {handler.doc}\n"
            elif handler.doc and 'admin' in handler.doc.lower():
                continue
            else:
                attack_text += f"{handler.commands[0]}: {handler.doc}\n"
    bot.reply_to(message, attack_text)
